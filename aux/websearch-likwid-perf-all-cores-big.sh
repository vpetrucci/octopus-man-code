...
#    taskset -c 5 /afs/cs.pitt.edu/usr0/vpetrucci/likwid-3.0/likwid-perfctr -o perf-small-$a.txt -t 1s -c 0 -g INSTR_RETIRED_ANY:FIXC0,L1D_REPL:PMC0,L2_LINES_IN_THIS_CORE_ALL:PMC1 &
    sleep 10 && taskset -c 5 /afs/cs.pitt.edu/usr0/vpetrucci/likwid-3.0/likwid-perfctr -o perf-$type-$a.txt -t 1s -c 2,3,4,5 -g INSTR_RETIRED_ANY:FIXC0,L2_RQSTS_SELF_MESI:PMC0,L2_LINES_IN_THIS_CORE_ALL:PMC1 &
  ...
  #sleep 1
     ...
     # sh /afs/cs.pitt.edu/usr0/vpetrucci/all-java-to-big-cores.sh   
	  ...
	  for i in $pid1; do taskset -c -p 2,3,4,5 $i; done
	  for i in $pid2; do taskset -c -p 2,3,4,5 $i; done
	  # set also for children
	
	  for i in $pid1; do
	    child_i=`ls /proc/$i/task/`
	    for j in $child_i; do taskset -c -p 2,3,4,5 $j; done
	  done
	  for i in $pid2; do
	    child_i=`ls /proc/$i/task/`
	    for j in $child_i; do taskset -c -p 2,3,4,5 $j; done
